...
def compute_trajectory(start_point, r, alpha, theta, beta, yff, E, unit_vector_3D, exit_unit_vector_3D, num_points=100, E_z=0):
    ...
    if E_z == 0:
        ...
        t_line = delta_y_line / (v * tangent_unit[1])

        x_line = np.array([x_arc[-1], x_arc[-1] + v * tangent_unit[0] * t_line])
        y_line = np.array([y_arc[-1], yff])
        z_line = np.array([z_arc[-1], z_arc[-1] + v * tangent_unit[2] * t_line])
        arc_mask = x_arc <= 0.18
...
energies_of_interest = np.arange(0.1, 1.55, 0.1)
delta_E = 0.05
...
energies_of_interest = np.arange(0.1, 1.7, 0.1)
delta_E = 0.05
...
for E_target in energies_of_interest:
    ...
    if len(x_vals) > 0:
        ...
        #y_text = np.min(zff_values) - 0.005
...
for i, E in enumerate(sorted_energies):
    ...
    if width > 0 and len(valid_vals) > 0:
        centers.append(np.mean(valid_vals))
        widths.append(width)
...
xff_values = np.array([intersection[2][0] for intersection in intersections])
energies = np.array([intersection[3] for intersection in intersections])
